...
local function notify(_, _, event)
	local delay = event == hs.window.filter.windowCreated and CREATE_DELAY or DESTROY_DELAY
	scheduleNotify(delay)
end
...
print("[window_watcher] windowCreated (250ms) + windowDestroyed/app terminated (50ms) → sketchybar")
