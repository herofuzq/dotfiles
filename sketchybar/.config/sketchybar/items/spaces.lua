	...
	sbar.exec(cmd, function(windows)
		...
		for i = 1, MAX_POPUP_SLOTS do
			...
			if i == 1 then
				-- 首行：工作区名称
				if item then
					item:set({ drawing = true, icon = { drawing = false }, label = { string = tostring(ws_index), color = appearance.colors.active.text, font = { family = fonts.font.text, style = fonts.font.style_map["Bold"], size = 16.0 }, padding_left = 50, padding_right = 50 } })
				end
			else
