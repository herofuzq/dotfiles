...
-- HOME 用于把仓库路径前的家目录折叠成 ~。
local home_prefix = os.getenv("HOME")
local function display_path(path)
	if not home_prefix or home_prefix == "" then return path end
	if path == home_prefix then return "~" end
	local child_prefix = home_prefix .. "/"
	if path:sub(1, #child_prefix) == child_prefix then
		return "~/" .. path:sub(#child_prefix + 1)
	end
	return path
end
...
local function render_popup(state)
	...
	for _, e in ipairs(state.entries) do
		...
		e.row:set({
			drawing = true,
			label = { string = icons.git .. "  " .. pad_label .. pad_branch .. pad_info .. display_path(e.path), color = status_color(e.status) },
		})
	end
