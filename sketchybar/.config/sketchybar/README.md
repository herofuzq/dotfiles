...
- Pure screen lock (no `system_will_sleep`) skips probes entirely and releases after a ~0.25s debounce (reset by repeated `screen_unlocked`); real system sleep uses one fast probe, and `display_change` switches back to full settling.
...
- 纯锁屏（没有 `system_will_sleep`）完全跳过 probe，约 0.25s debounce（重复 `screen_unlocked` 会重置）后直接渐入；真睡眠用一次快速 probe，`display_change` 才切回完整 settling。
