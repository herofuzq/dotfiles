...
- Pure lock/unlock: `system_woke` alone runs one fast probe after a ~0.25s debounce (reset by repeated `screen_unlocked`) and releases when unchanged; `display_change` switches back to full settling.
...
- 纯锁屏解锁时，单独出现的 `system_woke` 只做一次快速 probe（约 0.25s debounce，重复 `screen_unlocked` 会重置），无变化直接渐入；`display_change` 才切回完整 settling。
