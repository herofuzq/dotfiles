...
- Pure screen lock (no `system_will_sleep`) enters a ~1.2s cooldown after the first unlock; every later notification is ignored while the bar stays hidden, then it releases once. Real system sleep uses one fast probe, and `display_change` switches back to full settling.
...
- 纯锁屏（没有 `system_will_sleep`）从第一次解锁起进入约 1.2s 冷静期，期间后续通知全部忽略、bar 保持 hidden，结束再一次性渐入；真睡眠用一次快速 probe，`display_change` 才切回完整 settling。
